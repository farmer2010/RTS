        ...
        if self.can_shoot and self.reload_timer == 0 and self.target == None:
            for obj in self.world.objects:
                if obj._class == "unit" and obj.player != self.player:
                    if (obj.pos[0] - self.pos[0]) ** 2 + (obj.pos[1] - self.pos[1]) ** 2 <= (self.fog_radius * 16) ** 2:
                        self.target = obj
        if self.target != None and ((self.target.pos[0] - self.pos[0]) ** 2 + (self.target.pos[1] - self.pos[1]) ** 2 > (self.fog_radius * 16) ** 2 or self.target.killed):
            self.target = None
        if self.target != None and self.reload_timer == 0:
            rotate = math.atan2(self.target.pos[1] - self.pos[1], self.target.pos[0] - self.pos[0])
            self.world.objects.append(Projectile(self.world, self.player, self.pos.copy(), 10, 10, rotate, 5, self.fog_radius*16, 10))
